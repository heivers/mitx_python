...
class Digraph():
    ...
    edges is a dict mapping each node to a list fo its children"""
    def __init__(self):
        self._nodes = []
        self._edges = {}
    ...
    def __str__(self):
        ...
        return result[:-1] # omit last carriage return
...
def DFS(graph, start, end, path, shortest, to_print = False):
    """Assumes graph is aDigraph, start and end are nodes
    path and shortest are lists of notes.
...
def shortest_path(graph, start, end, to_print=False):
    """Assumes graph is a Digraph, start and end are nodes, 
    Returns a shortest path from start to en in graph"""#
